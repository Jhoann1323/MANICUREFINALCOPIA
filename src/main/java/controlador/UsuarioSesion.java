...
public class UsuarioSesion implements Serializable {
    ...
    public String validarUsuario() throws IOException {
        ...
        if (usuLog != null) {
            if (usuLog.getEstado() == Short.parseShort("0")) {
                    ...
                    if (usuLog.getContrasena().equals(claveIn)) {
                        switch (usuLog.getRolIdRol().getTipoRol()) {
                            case "Cliente":
                                ...
                                fc.getExternalContext().redirect("../faces/cliente/pv/user.xhtml");
                            case "Operario":
                                FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("correo", correoIn);
                                ...
                                fc.getExternalContext().redirect("../faces/empleado/pv/user.xhtml");
                            case "Administrador":
                                FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("correo", correoIn);
                                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Clave incorrecta", "Clave incorrecta"));
                                fc.getExternalContext().redirect("../faces/administrador/pv/user.xhtml");
                            default:
                                break;
                        }
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "El usuario inactivo", "El usuario inactivo"));
            return null;
        }
